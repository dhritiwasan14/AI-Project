...
def calc_loss(batch, net, tgt_net, device=device):
    ...
    states_v = torch.tensor(states).type(torch.float).to(device).squeeze(0)
    next_states_v = torch.tensor(next_states).type(torch.float).to(device)
...
if __name__ == "__main__":
    ...
    if PLAY:
        ...
        print(f"Running Pong (DQN_RAM) for {MAX_EPISODES}. Starting from Episode {EXTRA}") #declare start
        while True:
            frame_idx += 1
